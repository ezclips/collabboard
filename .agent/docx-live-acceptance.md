...
**Where provenance comes from, corrected.** An earlier draft argued the file
must be Word's because it carries parts like `people.xml`, `theme1.xml` and
`settings.xml`. **That is not proof and the argument is withdrawn** — any
writer could emit those parts, so their presence shows nothing about who wrote
the file. Provenance rests on the **documented creation process**: the
committed COM script, which is reproducible and inspectable. The XML is
evidence of something different and narrower — **which structures the specimen
actually exercises** — and that is all it is cited for below.
