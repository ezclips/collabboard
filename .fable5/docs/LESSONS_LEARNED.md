...
### A freshly started dev server times out unrelated e2e specs on first hit — not a regression (2026-07-08, PATCH-014 review)
**Symptom:** Codex reported the full e2e suite blocked by PATCH-014 with two failures in `settings-pages-render.spec.ts` (`/dashboard/settings/ai`, `/dashboard/settings/preferences`) — `net::ERR_ABORTED` / possible detached frame at `page.goto`. Neither page is touched by PATCH-014.
**Wrong path considered:** treating this as either "unrelated pre-existing flake, just rerun" or "a real blocking defect" without evidence for either — a coin-flip ruling would have been unjustified in both directions.
**Root cause:** the dev server had just been (re)started for this verification pass. Next.js dev mode compiles each route on-demand on its first request; on a cold server every request in that first full-suite run was markedly slower (board-lifecycle 49s vs. a normal 18–24s, smoke tests ~7s vs. ~1.5s) and the two settings pages simply lost the race against their fixed 30s `toBeVisible` timeout while compiling. Rerunning the same two specs alone (server now warm) passed in ~1s each; a second full-suite run on the warm server passed 18/18.
**Fix:** no code or spec change — this is a probing/verification-order issue, not a defect. Established as evidence, not assumption: reran standalone (proved warm-server timing), then reran the full suite (proved no interaction with PATCH-014's own new spec).
**Reusable rule:** the first full e2e run against a just-started dev server is not representative — a page/spec that fails only on a cold server with fixed 30s timeouts is not evidence of a regression by itself. Before ruling a failure blocking or unrelated, rerun once on a warm server; if it passes, the first run was a cold-start artifact, not a flake to chase and not a defect to fix.
