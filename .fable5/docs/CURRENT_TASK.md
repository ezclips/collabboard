...
| 052+ | hooks phase, remaining: the LAST postsRaw export retirement — `updatePadletById`/`updatePostRowById` (7 CanvasClient sites + the CanvasModals prop hand-off + 2 raw call sites INSIDE CanvasModals, per the 050 correction); then postsRaw.ts dies entirely (module deletion); FreeformPadletCards LAST on its own merits (NO postsRaw coupling — the 050 correction); realtime/presence CTO-only, undesigned | per-family; Pattern K where bound tests can carry semantics | PATCH-038→051 §0.1 rulings are the sequencing sources |
