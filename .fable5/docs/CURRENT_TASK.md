...
| 032+ | CanvasClient strangler, remaining groups per site map §7: nine deferred named UPDATE sites (swallow family: L903/L1604/L1621/L3097/L4108/L4133/L4398 — needs P3-family swallow command(s) + a settle-order ruling; check-and-branch pair: L3974/L4074 — needs an authorized micro-change ruling), then the 14 JSX UPDATE sites, the lone padlets select (hooks/read phase), auth trio (GPT-5.5, existing seams + one small updateUser command); then hooks (26 read sites); FreeformPadletCards LAST; realtime/presence CTO-only, undesigned | per-group; Pattern K where bound tests can carry semantics | site map §7 is the sequencing source |
