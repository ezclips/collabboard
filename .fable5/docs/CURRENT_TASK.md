...
| 033 | CanvasClient strangler group 8: TEN of the 14 JSX padlets UPDATE sites onto the EXISTING command quartet (7 bare-await sites → `updatePostMetadataBestEffort`; the check-and-branch `onUpdateChildComments` triplet → honest `updatePostMetadata` under an EXTENDED 032-Ruling-2 authorization, same convergence criteria verified per-site); the FIRST ONE-FILE patch of the program — zero domain changes, zero test changes, zero imports, zero new swallow sites (consumers of existing pins); the four non-fitting sites DEFERRED by column shape and named in the spec (2 position writes, 1 content+select map variant, 1 title write); the onDropExistingPadlet pair keeps first-throw-aborts-second; partial-handler swap at the detach site (position leg stays, 028 precedent); zero JSX structure churn; monolith 8,450→8,404, blank 727→730; padlets UPDATE 14→4; fidelity net = the existing 45 posts tests re-run green at authoring; spec fences SELF-VERIFIED (final hash reconstructed from the spec's own eight OLD/NEW pairs incl. the count==2 twin binding) | **GPT-5.4 acceptable** (Pattern K, ninth application: all swap shapes tsc --strict verified against the LIVE domain module) | **READY — `patches/PATCH-033.md`** |
| 034+ | CanvasClient strangler, remaining groups: the four deferred non-metadata writes (position pair → needs a position-write command; map comments variant + lone select → read-phase + content-carrying command; title write → title command or hook consolidation), auth trio (GPT-5.5, existing seams + one small updateUser command); then hooks (26 read sites); FreeformPadletCards LAST; realtime/presence CTO-only, undesigned | per-group; Pattern K where bound tests can carry semantics | site map §7 is the sequencing source |
...
| Authorize a fix for the canvas ops seam's error-swallow family — FOUR sites now: `canvas.reorderSections` (map reorder, PRESERVED by PATCH-026), `canvas.setChronoMode` (chrono-mode save, PRESERVED by PATCH-027), `canvas.attachPostToSchedulerContainer` and `canvas.createSchedulerContainerWithPost` (scheduler post/container inserts and the scheduler container's parentId update, PRESERVED verbatim by PATCH-029 — a five-statement bare-awaited cluster across three legacy handlers, ported as two command-internal swallows) — none repaired | User | Whenever (P3-family, non-blocking) | `lib/domain/canvas/sections.ts` (`reorderSections`), `lib/domain/canvas/board.ts` (`setChronoMode`), and `lib/domain/canvas/posts.ts` (`attachPostToSchedulerContainer`, `createSchedulerContainerWithPost`) — all await the write(s) and never read the resolved `error` field(s), same as the legacy handlers they replaced; only a thrown network error surfaces in any case. Test coverage exists for all four sites (each suite has a "preserves the legacy error-swallow"/"still returns ok" test) so a future fix changes one test's expectation per site, not the harness. |
