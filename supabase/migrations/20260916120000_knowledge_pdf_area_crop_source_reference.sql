-- KNOWLEDGE_PDF_AREA_CROP_SOURCE_REFERENCE_1: a PDF area crop writes its own
-- source reference.
--
-- THE DEFECT. A crop kept its provenance in ONE store only -- padlet metadata:
--
--     metadata.source = { kind: 'knowledge-pdf-area', knowledgeDocumentId,
--                         pageNumber, region: { x, y, width, height } }
--
-- while every consumer reads a DIFFERENT store, public.source_references: the
-- card's "Source . p. N" chip, its region preview, its click-back to the PDF,
-- and Board AI citation resolution. So the producer wrote where nobody reads.
-- A Note created from the same PDF region DOES get a reference, which is why a
-- Note shows its page and a crop of the very same rectangle shows nothing.
--
-- THE FIX IS ON THE PRODUCER SIDE. Both crop creators now write the region
-- reference in the SAME transaction that creates the placement, and the
-- backfill below gives the crops that already exist the reference they never
-- got. No consumer is changed by this file: the index, the preview and the
-- navigation are already reference-driven.
--
-- WHY A REGION REFERENCE IS THE RIGHT SHAPE, NOT A NEW ONE. The table already
-- models exactly this: a single page, all four region columns, no char span and
-- no quote. Five CHECK constraints enforce every part of it
...
--  source_references_char_range_check), so a malformed crop reference cannot be
-- stored at all. Nothing here relaxes or re-implements any of that.
--
-- ONE TRANSACTION, THREE STEPS, IN ORDER: replace the creator, replace the
-- reuse placement, then backfill. A failure at any point leaves the database
-- exactly as it was -- a half-applied state would mean new crops carry a
-- reference while the existing ones silently do not.
