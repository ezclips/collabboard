...
-- signal.
--
-- WHAT WAS WRONG. update_synced_note_pair set title, content and metadata and
-- left updated_at untouched on both members. padlets.updated_at is the only
-- change signal a post has, and there is no trigger on padlets that would
-- supply one, so an edited synced Note read as unedited to every consumer that
-- watches the column -- the board wiki's staleness check among them, which
-- versions a cited post by exactly this value and nothing else.
--
-- THIS IS THE SECOND HALF OF ONE DEFECT. The unsynced-Note branch of
-- usePadletSave had the same omission and was fixed in the application layer;
-- every sibling editor (link, todo, table, drawing, card) already stamped. This
-- migration closes the remaining instance, which lives in SQL because the
-- synced pair writes through this function rather than through PostgREST.
--
-- REPLACEMENT, NOT REDESIGN. CREATE OR REPLACE with one added assignment and
-- its comment. Every guard, lock, allowlist, reciprocity check and row-count
-- assertion below is byte-identical to the reviewed 20260913120000 body: the
-- authorization argument is unchanged, and the verify file's body digest is the
-- proof of that claim rather than this sentence.
--
-- ---------------------------------------------------------------------------
-- The reviewed function documentation from 20260913120000 follows unchanged.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    --
    -- SYNCED_NOTE_PAIR_UPDATED_AT_1: updated_at is set here, and it was not
    -- before. padlets.updated_at is the only signal a post carries that it
    -- changed; a write that rewrites title and content without moving it
    -- reports the row as unchanged to everything that watches posts. now() is
    -- transaction time, so BOTH members take the identical stamp -- which is
    -- the truth about a pair that moves together or not at all.
...
COMMENT ON FUNCTION public.update_synced_note_pair(uuid, uuid, text, text, jsonb, jsonb) IS
    'SYNCED_NOTE_PAIR_ATOMIC_UPDATE_1 + SYNCED_NOTE_PAIR_UPDATED_AT_1: updates both members of a reciprocal synced Note pair in one transaction. SECURITY INVOKER; auth.uid() only; RLS authoritative. Synchronizes title, content and four Note-editor appearance keys, merged into each row''s own metadata, and stamps updated_at on both members so the pair''s change is visible to everything that watches padlets.updated_at.';
