-- CollabBoard SYNCED_NOTE_PAIR_ATOMIC_UPDATE_1 production rollout.
--
-- SOURCE:
--   supabase/migrations/20260913120000_update_synced_note_pair.sql
--
-- The function and grants below are a byte-faithful copy of that reviewed
...
--
-- WHAT IT FIXES. A synced Note pair was updated by two sequential PostgREST
-- writes: anything between them left the pair PERMANENTLY split, and the twin
-- additionally had its metadata REPLACED by the edited member's object. One
-- transaction removes the window; per-row merging removes the overwrite.
--
-- WHAT IT DOES NOT TOUCH: no table, column, index, constraint, policy, trigger,
-- grant on any relation, bucket or storage object is created, altered or
-- dropped, and no other function is redefined. Existing RLS is untouched and
-- stays authoritative -- SECURITY INVOKER adds no privilege.
--
-- ROW WRITES: NONE. This rollout creates one function and sets its execute
-- privileges. It BACKFILLS NOTHING: existing pairs, malformed legacy ones
-- included, are left exactly as they are and fail closed at call time.
--
-- REVERSIBILITY. DROP FUNCTION public.update_synced_note_pair(uuid, uuid, text,
-- text, jsonb, jsonb) is a complete rollback: the client treats a missing
-- function as a typed failure, keeps the draft and writes no row, and the
-- function owns no data to orphan.
--
-- VERIFY with the read-only companion file:
--   20260913120000_synced_note_pair_atomic_update_verify.sql
