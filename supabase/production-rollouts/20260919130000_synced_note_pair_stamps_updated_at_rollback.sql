-- ROLLBACK for 20260919130000_synced_note_pair_stamps_updated_at.sql.
--
-- WHAT IT DOES. Restores the function body reviewed and applied as
-- 20260913120000, verbatim -- every guard, lock, allowlist and assertion
-- included. It is a CREATE OR REPLACE, so nothing is dropped, no row is
-- touched, and no data is lost. Running it against a database that never got
-- 20260919130000 is a no-op in effect: it writes the body that is already
-- there.
--
-- WHAT YOU GIVE UP BY RUNNING IT. A synced Note save stops moving
-- padlets.updated_at again. That is not cosmetic: updated_at is the only
-- change signal a post carries, so every consumer that watches it -- the board
-- wiki's source-staleness check first among them -- goes back to reading an
-- edited synced Note as unedited. The stamps already written by the newer body
-- STAY; only future saves stop stamping. So a wiki page that went stale because
-- of a synced-Note edit does not un-stale itself, and nothing needs repairing
-- after a rollback.
--
-- WHEN RUNNING THIS IS THE RIGHT CALL: the newer body misbehaves in a way that
-- is worse than the missing stamp -- a failing save, a lock or permission
-- surprise. It is NOT the response to "the wiki says a page is stale": that is
-- the fixed behaviour working.
--
-- AFTER RUNNING IT, the 20260913120000 verify file passes again (its body
-- digests are the ones this file restores), and the 20260919130000 verify file
-- fails at its body-digest row -- which is the correct and intended reading.
