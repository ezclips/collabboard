...
import type { AIRole } from '@/lib/ai/aiRoles';
import {
  fetchAIProviders,
  fetchAIRoles,
  saveAIRole,
 ...
 * executing route never learns of this: it resolves the role per request and
 * finds whatever the user last chose.
 *
 * That is also why no request carries a provider, model or key. A chooser that
 * sent its selection with the message would be a second, weaker authority over
 * execution -- one a browser could set to anything.
 *
 ...
 *
 * ------------------------------------------------------------------
 * IT MUST NEVER NAME A PROVIDER IT HAS NOT READ, which is the whole reason the
 * status is three-valued rather than a boolean. Until both fetches resolve it
 ...
 * during it reported the managed default while the role's stored preference was
 * a BYOK connection. A definite wrong value is worse than no value.
 *
 * "Could not load" is therefore NOT "loaded, and there is nothing". A failed
 * load leaves the STORED preference in force, whatever this component failed to
 * read, so it may not claim a provider and must not offer to change one.
 * ------------------------------------------------------------------
 *
 * WHY THIS IS GENERIC. Every AI surface resolves its model from a role, and a
 * user who cannot see or change that from the surface has to go hunting in
 * Settings. One component, parameterised by role, keeps the honesty rules above
 * in a single place -- they were each learned from a specific defect, and a
 * second copy is where they get lost.
 */

export interface AIRoleModelChooserProps {
  /** The role whose stored preference this chooser reads and writes. */
  readonly role: AIRole;
  /** Accessible name, e.g. "Board Chat model". */
  readonly label: string;
  /**
   * Data-attribute namespace, e.g. `board-ai-chat` yields
   * `data-board-ai-chat-model` and `data-board-ai-chat-model-status`. Each
   * surface keeps its own so a test can address one chooser unambiguously.
   */
  readonly attributePrefix: string;
  /** What the caller is told when a save fails. Surface-specific wording. */
  readonly saveErrorMessage: string;
  /** Disabled while a request is in flight, so a swap cannot race it. */
  readonly disabled?: boolean;
  /** Surfaced by the host, which owns its one error line. */
...
export default function AIRoleModelChooser({
  role,
  label,
  attributePrefix,
  saveErrorMessage,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setSelected(roles[role]?.connectionId ?? DEFAULT_VALUE);
        setStatus('ready');
      } catch {
        // See the header: the route resolves this role per request and finds the
        // preference that is actually stored. Because the value never changed,
        // picking the option it is already showing fires no change event and
        // would persist nothing -- so disabled is the honest control.
  ...
  const choose = useCallback(async (connectionId: string) => {
    ...
    try {
      ...
      await saveAIRole(role, connectionId === DEFAULT_VALUE ? null : connectionId, null);
    } catch {
  ...
  }, [selected, onError, role, saveErrorMessage]);
  ...
  return (
    <label
      ...
      {...{ [`data-${attributePrefix}-chooser`]: 'true' }}
    >
      <span className="sr-only">{label}</span>
      <select
        aria-label={label}
        {...{
          [`data-${attributePrefix}-model`]: '',
          [`data-${attributePrefix}-model-status`]: status,
        }}
        ...
        {status === 'ready' ? (
          <>
            ...
            {connections.map((connection) => (
              ...
              // question, and it put credential-adjacent material in the UI for
              // no reason.
