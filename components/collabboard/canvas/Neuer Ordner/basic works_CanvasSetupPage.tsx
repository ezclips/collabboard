...
const PreviewSection: React.FC<{
...
}> = ({ layout, columns, newPostsAtTop, layoutTypes, onEditItem }) => {
  const selectedLayoutInfo = layoutTypes.find(l => l.id === layout);
  ...
  const renderPreview = () => {
    if (layout === 'columns') {
              ...
              <div className="space-y-2">
                {col.items.map((item) => (
                  <div
                    key={item.id}
                    className="bg-white p-3 rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                    onClick={() => onEditItem(item, col.id)}
                  >
                    <h4 className="font-medium text-sm mb-1">{item.title}</h4>
      ...
      if (displayedPadlets.length === 0) {
        return (
          <div className="flex items-center justify-center h-64 text-gray-500">
            Add some content to see the {selectedLayoutInfo?.name} layout preview
          </div>
        );
      }
      ...
      let positions;
      if (layout === 'wall' || layout === 'stream') {
        positions = calculatePositions(displayedPadlets.length, previewWidth);
      } else {
        positions = calculatePositions(displayedPadlets.length, previewWidth, previewHeight);
      }

      return (
        <div className="relative h-full w-full p-4" style={{ minHeight: '400px' }}>
          {displayedPadlets.map((padlet, index) => {
            ...
            return (
              <div
                key={padlet.id}
                className="absolute bg-white p-3 rounded-lg shadow-sm border cursor-pointer hover:shadow-md transition-shadow"
                style={{
                  top: `${pos.top}px`,
                  left: `${pos.left}px`,
                  width: `${pos.width}px`,
              ...
              >
                <h4 className="font-medium text-sm mb-1">{padlet.title}</h4>
                <p className="text-xs text-gray-600">{padlet.content}</p>
              </div>
            );
          })}
        </div>
  ...
  return (
    <Card className="bg-white/95 backdrop-blur-md border border-gray-200 shadow-lg" suppressHydrationWarning>
      <CardHeader className="bg-white/90">
        <CardTitle className="text-gray-900">Preview - {selectedLayoutInfo?.name}</CardTitle>
      </CardHeader>
      <CardContent className="bg-white/90" suppressHydrationWarning>
        <div className="border rounded-lg bg-gray-50 h-64 overflow-auto">
          {renderPreview()}
        </div>
      </CardContent>
    </Card>
...
const CanvasSetupPage: React.FC = () => {
  ...
  const layoutTypes = useMemo(() => {
    const unique = new Map<string, typeof importedLayoutTypes[number]>();
    
    importedLayoutTypes.forEach((layout) => {
      if (layout.id === "timeline") return; // Omit problematic timeline
  ...
  const [columns, setColumns] = useState<ColumnData[]>([
    { 
      ...
      title: 'Ideas', 
      items: [ 
        { id: 'item-1', title: 'Welcome!', content: 'This is your new canvas. Add content and arrange it however you like.' }, 
        { id: 'item-2', title: 'Collaborate', content: 'Invite team members to work together in real-time.' } 
      ] 
    },
    { 
      id: 'column-2', 
      title: 'In Progress', 
      items: [ 
        { id: 'item-3', title: 'Organize', content: 'Switch between different layouts to organize your content.' } 
      ] 
    },
    { 
      id: 'column-3', 
      title: 'Done', 
      items: [
        { id: 'item-4', title: 'Customize', content: 'Change colors, wallpapers, and settings to match your style.' }
      ] 
    }
  ]);
