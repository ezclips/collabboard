...
export const Popover = ({
  ...
  useLayoutEffect(() => {
    if (fitInViewport && popoverRef.current && top != null && left != null) {
      ...
      // Switch to fixed positioning so the popover escapes overflow:hidden ancestors.
      container.style.position = "fixed";

      // Convert container-relative coords to viewport-relative.
      const vpLeft = left + offsetLeft;
      const vpTop = top + offsetTop;
      ...
      } else if (vpLeft + width > vw) {
        container.style.left = `${vw - width - 10}px`;
      } else {
        container.style.left = `${vpLeft}px`;
      }
      ...
      } else if (vpTop + height > vh) {
        // Flip above click point (standard context menu behaviour).
        // Fall back to top=0 if there isn't enough room above either.
        container.style.top = `${Math.max(0, vpTop - height)}px`;
      } else {
        container.style.top = `${vpTop}px`;
      }
    }
  }, [
    top,
