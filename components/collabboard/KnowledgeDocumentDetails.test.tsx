...
/**
 * PATCH-159. KEPT FROM PATCH-157, and it has nothing to do with transcripts.
 *
 * A citation whose focus range is cut by a separate highlight is emitted as
 * several focused pieces on ANY document, PDF included. All of them ring and
 * carry the navigation marker -- the emphasis is shared; only the scroll
 * anchor is confined. Nothing else pinned that pre-existing behaviour, which
 * is how PATCH-157's intermediate version narrowed the emphasis to a single
 * piece without a single test noticing. This is the guard that was missing.
 */
describe('a citation split into segments shares its emphasis (PDF path)', () => {
  ...
  it('12. REGRESSION GUARD: a citation split into two segments rings BOTH', () => {
    ...
    // emphasis, which is behaviour the PDF path had before PATCH-157.
    const wideRef = {
      id: 'ref-wide',
      sourceDocumentId: DOC_ID,
      pageStart: 1,
