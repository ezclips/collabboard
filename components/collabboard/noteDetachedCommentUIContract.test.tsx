...
describe('normal/detached Note comments — canonical Comment UI v1 migration', () => {
  // PATCH 8AF: FreeformPadletCards.tsx used to have TWO "Note detached
  // comments" entry points -- an "on-canvas badge popup" (live, gated on
  // cardCommentPopupPadletId === padlet.id) and a "toolbar popup" (gated on
  // BOTH that AND cardToolbarPadletId, which turned out to have no live
  // non-null setter anywhere in the codebase -- see
  // commentPermissionClosure.contract.test.tsx's PATCH 8AF describe block).
  // The dead second entry point was removed; only the live on-canvas badge
  // popup remains.
  it('routes the one live detached Note entry point through CommentPopup', () => {
    ...
    expect(freeform.match(/Note detached comments[\s\S]*?<CommentPopup/g)?.length).toBe(1);
  });
  ...
  it('removes local detached Note rows, action rails, composers, and pickers', () => {
    ...
    expect(detachedPanels).toHaveLength(1);
  ...
  // Proves the pre-existing interaction-isolation wrapper around the live
  // Note CommentPopup site is still present, so a click/mousedown inside the
  // comment panel cannot leak through to the card/canvas underneath (drag,
  // selection, parent-card click). This wrapper predates PATCH 8P; this
  // test simply closes the gap that nothing previously guarded it.
  //
  // PATCH 8AF removed the second ("toolbar popup") site this test used to
  // also check -- it was gated on cardToolbarPadletId, which has no live
  // non-null setter anywhere in the codebase, so it was never actually
  // reachable. See commentPermissionClosure.contract.test.tsx's PATCH 8AF
  // describe block.
  it('the live Note CommentPopup site is wrapped in a click/mousedown isolation container', () => {
    const freeform = read(FREEFORM);
    const anchor = 'Note detached comments use the canonical panel; this shell only owns placement and close state.';
