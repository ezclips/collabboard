...
export default function CommentEditor({
    ...
    // Reset & cleanup editors when modal opens / closes
    useEffect(() => {
        if (isOpen) {
            // Reset all UI state
            ...
            // Force new editor instances
            setEditorKey((prev) => prev + 1);

            // Clear content after tiny delay (TipTap needs time to initialize)
            setTimeout(() => {
                if (editor && !editor.isDestroyed) {
                    editor.commands.clearContent(true); // true = keep history cleared too
        ...
        // Cleanup when modal closes (extra safety)
        return () => {
            if (editor && !editor.isDestroyed) {
    ...
    }, [isOpen, initialComments, initialCardColor, editor, editEditor]);
                                            ...
                                            <div className="flex-1">
                                                ...
                                                {editingCommentId === comment.id && editEditor ? (
                                                    <div className="flex flex-col gap-2">
                                                        <EditorContent key={editorKey} editor={editEditor} />
