...
describe('AI_TIME_BUDGETS is bound to the server deadlines', () => {
  ...
  it('the table-fill route waits 25 s, as the table says', () => {
    const source = read('app/api/ai/table-fill/route.ts');
    const match = /setTimeout\(\s*\(\)\s*=>\s*controller\.abort\(\),\s*([\d_]+)\s*\)/.exec(source);
    expect(match, 'no abort timeout found in the table-fill route').not.toBeNull();
    expect(Number((match![1]).replace(/_/g, '')) / 1000).toBe(budgetFor('Table fill with AI'));
  });

  it('the table-plan route waits 25 s, as the table says', () => {
    const source = read('app/api/ai/table-plan/route.ts');
    const match = /setTimeout\(\s*\(\)\s*=>\s*controller\.abort\(\),\s*([\d_]+)\s*\)/.exec(source);
    expect(match, 'no abort timeout found in the table-plan route').not.toBeNull();
    expect(Number((match![1]).replace(/_/g, '')) / 1000).toBe(budgetFor('Edit table with AI'));
  });

  it('the table-from-document route waits 60 s, as the table says', () => {
    const source = read('app/api/boards/[id]/ai/table-from-document/route.ts');
    const match = /setTimeout\(\s*\(\)\s*=>\s*controller\.abort\(\),\s*([\d_]+)\s*\)/.exec(source);
    expect(match, 'no abort timeout found in the table-from-document route').not.toBeNull();
